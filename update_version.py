...
class Updater():
    ...
    def readLines(self):
        ...
        with open(paths + "\\version.rc", "r") as f:
            ...
            self.filevers = line.split("=")[1].replace('(', '')\
                .replace(')', '').strip()
            self.filevers = self.filevers[0:self.filevers
                                          .__len__() - 1]
            ...
            self.ProductVersion =\
                line4.split(",")[1].replace('\'', '').replace(')', '')\
                .replace('u', '').strip()
            self.ProductVersion = self.ProductVersion[0:self.ProductVersion
                                                      .__len__()]
            f.close()
    ...
    def writeLines(self):
        with open(paths + "\\Functions.py", "w") as w:
            ...
            self.lines[pos] = self.lines[pos].replace(self.old,
                                                      ''.join(["v", self.new]))
        ...
        with open(paths + "\\version.rc", "w") as w:
            ...
            self.vlines[pos1] = self.vlines[pos1].replace(self.filevers,
                                                          self.new.replace('.',
                                                                           ',')
                                                          )
            self.vlines[pos2] = self.vlines[pos2].replace(self.prodvers,
                                                          self.new.replace('.',
                                                                           ',')
                                                          )
            self.vlines[pos3] = self.vlines[pos3].replace(self.FileVersion,
                                                          self.new)
            self.vlines[pos4] = self.vlines[pos4].replace(self.ProductVersion,
                                                          self.new)
